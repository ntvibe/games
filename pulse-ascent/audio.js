...
export class AudioCore {
  ...
  async init(){
    if(this.started)return;this.ctx=new(window.AudioContext||window.webkitAudioContext)({latencyHint:'interactive'});await this.ctx.resume();
  ...
  osc(type,freq,t,dur,gain,dest=this.music,detune=0){const o=this.ctx.createOscillator(),g=this.ctx.createGain();o.type=type;o.frequency.setValueAtTime(freq,t);o.detune.value=detune;g.gain.setValueAtTime(.0001,t);g.gain.exponentialRampToValueAtTime(Math.max(.0002,gain),t+.006);g.gain.exponentialRampToValueAtTime(.0001,t+dur);o.connect(g);g.connect(dest);o.start(t);o.stop(t+dur+.03);return{o,g}}
  ...
  pluck(t,midi,g=.08,dur=.12){const f=this.midi(midi),{g:gain}=this.osc('triangle',f,t,dur,g,this.music);this.osc('sine',f*2,t,dur*.55,g*.22,this.fx,rand(-4,4));return gain}
  pad(t,midi,dur,g=.04){const f=this.midi(midi);[-9,0,8].forEach(det=>{const o=this.ctx.createOscillator(),gain=this.ctx.createGain(),filter=this.ctx.createBiquadFilter();o.type='sawtooth';o.frequency.value=f;o.detune.value=det;filter.type='lowpass';filter.frequency.value=900+this.energy*1400;filter.Q.value=.7;gain.gain.setValueAtTime(.0001,t);gain.gain.linearRampToValueAtTime(g/3,t+.18);gain.gain.linearRampToValueAtTime(.0001,t+dur);o.connect(filter);filter.connect(gain);gain.connect(this.music);o.start(t);o.stop(t+dur+.05)})}
  ...
  lockNote(index){if(!this.ctx)return;const t=this.ctx.currentTime,m=this.rootMidi+24+this.scale[index%this.scale.length];this.osc('sine',this.midi(m),t,.065,.075,this.fx)}
  shotNote(index,time,mult=1){if(!this.ctx)return;const m=this.rootMidi+24+this.scale[index%this.scale.length]+(this.section>=3?12:0),f=this.midi(m);this.osc('sine',f,time,.16,.10*mult,this.fx);this.osc('triangle',f*2,time,.07,.035*mult,this.fx,rand(-6,6))}
