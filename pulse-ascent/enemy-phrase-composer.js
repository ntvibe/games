...
waitFor().then(game=>{
  ...
  const state={phrases:0,steps:0,responseShots:0,resolutions:0,lastArea:1,lastName:'',lastVoice:0,lastStep:-1};
  ...
  function spawnResponse(source,{targetX,targetY,speed=23.5,radius=.78,color}){
    ...
    threat.__areaAttackSignature=true;threat.__enemyPhraseResponse=true;threat.__enemyPhraseName=PROFILES[area()].name;
  ...
  function responseShot(a,pool,avatar,p){
    ...
    if(a===0){const src=sourceBySide(1,pool);return !!spawnResponse(src,{targetX:avatar.x+.82,targetY:avatar.y-.24,speed:23.4+p*.55,radius:.8,color:PROFILES[a].color});}
    if(a===1){const src=sourceBySide(1,pool);return !!spawnResponse(src,{targetX:avatar.x-.78,targetY:avatar.y-.3,speed:23.6+p*.6,radius:.8,color:PROFILES[a].color});}
    if(a===2){const src=pool[(state.phrases*2+1)%pool.length];return !!spawnResponse(src,{targetX:avatar.x+.12,targetY:avatar.y,speed:22.2+p*.52,radius:.77,color:PROFILES[a].color});}
    if(a===3){const src=pool[(state.phrases+1)%pool.length],spread=1.15+p*.14;return !!spawnResponse(src,{targetX:avatar.x+spread,targetY:avatar.y-.36,speed:23.6+p*.58,radius:.76,color:PROFILES[a].color});}
    const src=sourceBySide(1,pool);return !!spawnResponse(src,{targetX:avatar.x+.92,targetY:avatar.y-.32,speed:24+p*.58,radius:.75,color:PROFILES[a].color});
  }
  ...
  function playStep(step,time){
    ...
    tone(a,voice,time);
    let fired=true;
    // Existing Area attacks provide the call and resolution. We add one restrained
    // response voice in the middle, turning the bar into a musical combat phrase
    // without doubling the established danger cadence.
    if(voice===1){
      const baseAlreadyOwnsMiddle=(a===2&&phase()>=3);
    ...
    dispatchEvent(new CustomEvent('pulse:enemy-phrase-step',{detail:{area:a+1,name:profile.name,voice:voice+1,step:s,resolution:voice===2,response:voice===1}}));
    return true;
  }
