...
function buildTethers(game){
  ...
  const pp=new Float32Array(count*3),pg=new THREE.BufferGeometry();pg.setAttribute('position',new THREE.BufferAttribute(pp,3));
  const pm=new THREE.PointsMaterial({color:0xff6bd8,size:mobile()?.07:.055,transparent:true,opacity:.7,blending:THREE.NormalBlending,depthWrite:false,sizeAttenuation:true});
  const packets=new THREE.Points(pg,pm);packets.frustumCulled=false;game.scene.add(packets);
...
waitFor().then(game=>{
  ...
  game.world.update=(dt,t,energy,sync=0)=>{
    ...
    const charge=mode==='charge'?1:(mode==='recover'?0.45:0);
    ...
    const w=topology.worlds?.[state.area],attr=w?.geo?.attributes?.position,activeVertices=Math.max(2,Math.min(attr?.count||2,w?.geo?.drawRange?.count||attr?.count||2));
    const living=boss.parts.filter(p=>!p.dead),activeTethers=Math.min(state.tethers.count,[4,7,10][boss.phase-1]||10);
    if(w&&attr&&living.length){
      const src=new THREE.Vector3(),end=new THREE.Vector3();
      for(let i=0;i<activeTethers;i++){const vi=Math.min(activeVertices-1,Math.floor(((i+.35)/activeTethers)*activeVertices)),j=i*6;src.set(attr.getX(vi),attr.getY(vi),attr.getZ(vi));w.root.localToWorld(src);living[i%living.length].pivot.getWorldPosition(end);state.tethers.pos[j]=src.x;state.tethers.pos[j+1]=src.y;state.tethers.pos[j+2]=src.z;state.tethers.pos[j+3]=end.x;state.tethers.pos[j+4]=end.y;state.tethers.pos[j+5]=end.z;const f=(t*(.34+.08*boss.phase)+i*.113)%1,k=i*3;state.tethers.pp[k]=lerp(src.x,end.x,f);state.tethers.pp[k+1]=lerp(src.y,end.y,f);state.tethers.pp[k+2]=lerp(src.z,end.z,f);}
      ...
      w.mat.opacity=Math.min(.22,(w.mat.opacity||.1)+phaseN*.025+charge*.018);w.packetMat.opacity=Math.min(.82,(w.packetMat.opacity||.4)+phaseN*.04+charge*.05);w.root.scale.setScalar(1+Math.sin(t*.55)*.006+state.phaseShock*.018);if(state.phaseShock>.04)w.root.rotation.z+=Math.sin(t*5.5)*state.phaseShock*.0025;
    }
  };
