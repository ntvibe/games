...
export class Boss {
  constructor(game){
    ...
    this.core=new THREE.Mesh(new THREE.IcosahedronGeometry(2.55,2),new THREE.MeshBasicMaterial({color:0xffffff,wireframe:true,transparent:true,opacity:.52,blending:THREE.AdditiveBlending}));this.group.add(this.core);
    this.innerCore=new THREE.Mesh(new THREE.OctahedronGeometry(.75,1),new THREE.MeshBasicMaterial({color:0xfff6ff,wireframe:true,transparent:true,opacity:.85,blending:THREE.AdditiveBlending}));this.group.add(this.innerCore);
    for(let r=0;r<5;r++){const tor=new THREE.Mesh(new THREE.TorusGeometry(3.5+r*.72,.035,4,72),new THREE.MeshBasicMaterial({color:r%2?0xff5fd5:0x6ff7ff,transparent:true,opacity:.28,blending:THREE.AdditiveBlending}));tor.rotation.x=rand(0,TAU);tor.rotation.y=rand(0,TAU);this.group.add(tor);this.rings.push(tor);}
    this.spokes=[];for(let i=0;i<8;i++){const pivot=new THREE.Group(),m=new THREE.Mesh(new THREE.OctahedronGeometry(.62,0),new THREE.MeshBasicMaterial({color:i%2?0xff68d7:0x66eeff,wireframe:true,transparent:true,opacity:.92,blending:THREE.AdditiveBlending}));pivot.add(m);const spoke=new THREE.Mesh(new THREE.BoxGeometry(.035,4.4,.035),new THREE.MeshBasicMaterial({color:i%2?0xff68d7:0x66eeff,transparent:true,opacity:.16,blending:THREE.AdditiveBlending}));spoke.position.y=2.2;pivot.add(spoke);pivot.userData.index=i;this.group.add(pivot);this.parts.push({pivot,mesh:m,locked:false,dead:false,hp:15,worldPos:new THREE.Vector3()});this.spokes.push(spoke);}
    game.particles.burst(this.group.position,320,0x8bf5ff,17,20);game.audio.sectionStab(4);
  }
  ...
  update(dt,t){
    if(this.dead)return;this.age+=dt;this.group.position.z=lerp(this.group.position.z,-25,dt*.32);this.group.rotation.z+=dt*.065*this.phase;this.core.rotation.x+=dt*.24;this.core.rotation.y+=dt*.4;this.innerCore.rotation.x-=dt*.72*this.phase;this.innerCore.rotation.z+=dt*.95;const beat=t*SETTINGS.bpm/60,pulse=Math.pow(Math.max(0,Math.cos((beat%1)*TAU)),10);this.core.scale.setScalar(1+pulse*.1*this.phase);this.innerCore.scale.setScalar(.86+pulse*.38);
    this.rings.forEach((r,i)=>{r.rotation.z+=dt*(.08+i*.025)*(i%2?-1:1)*this.phase;r.material.opacity=lerp(r.material.opacity,.2+pulse*.14+(this.phase-1)*.04,dt*5);r.scale.setScalar(1+pulse*.02*(i+1));});
