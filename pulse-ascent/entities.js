...
export class Boss {
  constructor(game){
    ...
    this.core=new THREE.Mesh(new THREE.IcosahedronGeometry(1.72,1),new THREE.MeshBasicMaterial({color:0x62e6ff,wireframe:true,transparent:true,opacity:.2,blending:THREE.NormalBlending,depthWrite:false}));this.group.add(this.core);
    this.innerCore=new THREE.Mesh(new THREE.OctahedronGeometry(.48,0),new THREE.MeshBasicMaterial({color:0xff66d8,wireframe:true,transparent:true,opacity:.48,blending:THREE.NormalBlending,depthWrite:false}));this.group.add(this.innerCore);
